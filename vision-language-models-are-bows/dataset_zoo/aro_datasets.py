...
class VG_Relation(Dataset):
    ...
    def download_annotation(self, annotation_file):
        # Initial request to get the confirm token
        google_drive_url = "https://drive.google.com/uc?export=download&id=1kX2iCHEv0CADL8dSO1nMdW-V0NqIAiP3"
        ...
        google_drive_url_with_token = google_drive_url
...
class VG_Attribution(Dataset):
    ...
    def download_annotation(self, annotation_file):
        # Initial request to get the confirm token
        google_drive_url = "https://drive.google.com/uc?export=download&id=13tWvOrNOLHxl3Rm9cR3geAdHx2qR3-Tw"
        session = requests.Session()
        # response = session.get(google_drive_url, stream=True)
        # soup = BeautifulSoup(response.content, 'html.parser')
        # confirm_token = soup.find("a", href=True)['href'].split('=')[-1]
